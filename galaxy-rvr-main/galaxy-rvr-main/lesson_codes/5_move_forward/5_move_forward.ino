#include <SoftPWM.h>

// Define the pins of motors
const int in1 = 2;
const int in2 = 3;
const int in3 = 4;
const int in4 = 5;

// Define the pins for the IR modules
#define IR_RIGHT 7
#define IR_LEFT 8

// Define the pin for the ultrasonic module
#define ULTRASONIC_PIN 10

// Variables for spiral movement
int baseSpeed = 150;  // Base speed for the motors
int turnSpeed = 50;   // Initial turning speed
int turnIncrement = 5; // Increment for the turning speed
int maxTurnSpeed = 200; // Maximum turning speed

void setup() {
  // Initialize SoftPWM
  SoftPWMBegin();

  // Set the IR module pins as inputs
  pinMode(IR_RIGHT, INPUT);
  pinMode(IR_LEFT, INPUT);

  // Set the ultrasonic pin
  pinMode(ULTRASONIC_PIN, OUTPUT);

  // Initialize serial communication for debugging
  Serial.begin(9600);
}

void loop() {
  // Read IR sensor values
  int rightIR = digitalRead(IR_RIGHT);
  int leftIR = digitalRead(IR_LEFT);

  // Read distance from the ultrasonic sensor
  float distance = readUltrasonic();

  // Check for obstacles
  if (distance < 20 || (rightIR == 0 && leftIR == 0)) {
    // Obstacle detected, move backward and turn
    moveBackward(baseSpeed);
    delay(500);
    turnLeft(baseSpeed);
    delay(500);
  } else if (rightIR == 0) {
    // Right IR sensor blocked, turn left
    turnLeft(baseSpeed);
  } else if (leftIR == 0) {
    // Left IR sensor blocked, turn right
    turnRight(baseSpeed);
  } else {
    // No obstacles, move in a spiral pattern
    moveSpiral();
  }
}

void moveSpiral() {
  // Gradually increase the turning speed to create a spiral pattern
  moveForward(baseSpeed);
  SoftPWMSet(in4, baseSpeed - turnSpeed);  // Adjust right motor speed for turning
  turnSpeed += turnIncrement;
  if (turnSpeed > maxTurnSpeed) {
    turnSpeed = 50;  // Reset turning speed after reaching the maximum
  }
  delay(500);
}

float readUltrasonic() {
  // A 4ms delay is required, otherwise the reading may be 0
  delay(4);

  // Set to OUTPUT to send signal
  pinMode(ULTRASONIC_PIN, OUTPUT);

  // Clear the trigger pin
  digitalWrite(ULTRASONIC_PIN, LOW);
  delayMicroseconds(2);

  // Trigger the sensor by sending a high pulse for 10us
  digitalWrite(ULTRASONIC_PIN, HIGH);
  delayMicroseconds(10);

  // Set the trigger pin back to low
  digitalWrite(ULTRASONIC_PIN, LOW);

  // Set to INPUT to read
  pinMode(ULTRASONIC_PIN, INPUT);

  // pulseIn returns the duration of the pulse on the pin
  float duration = pulseIn(ULTRASONIC_PIN, HIGH);

  // Calculate the distance (in cm) based on the speed of sound (340 m/s or 0.034 cm/us)
  float distance = duration * 0.034 / 2;

  return distance;
}

void moveForward(int speed) {
  // Set the left motors rotate counterclockwise
  SoftPWMSet(in1, speed);
  SoftPWMSet(in2, 0);

  // Set the right motors rotate clockwise
  SoftPWMSet(in3, 0);
  SoftPWMSet(in4, speed);
}

void moveBackward(int speed) {
  // Set the left motors rotate clockwise
  SoftPWMSet(in1, 0);
  SoftPWMSet(in2, speed);

  // Set the right motors rotate counterclockwise
  SoftPWMSet(in3, speed);
  SoftPWMSet(in4, 0);
}

void turnLeft(int speed) {
  // Set all motors to rotate clockwise
  SoftPWMSet(in1, 0);
  SoftPWMSet(in2, speed);
  SoftPWMSet(in3, 0);
  SoftPWMSet(in4, speed);
}

void turnRight(int speed) {
  // Set all motors to rotate counterclockwise
  SoftPWMSet(in1, speed);
  SoftPWMSet(in2, 0);
  SoftPWMSet(in3, speed);
  SoftPWMSet(in4, 0);
}